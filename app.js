...
const { token, naughtyStudents, specialStudents } = require('./config.json');

// 全部的句的檔案在 quotes.json 裡
// naughtyStudents 是特別調皮的同學們的 Discord IDs
const { scoldings, motivation, extra, special } = require('./quotes.json');
...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === 'scold') {
    ...
    else if (specialStudents.includes(interaction.user.id)) {
      await interaction.reply(getRandomItem(special));
    }
    else {
    // 給其他的同學們一般的「關照」
